...
public sealed class ReportService(PdvDbContext db)
{
    ...
    public async Task<RevenueByDayEntry[]> GetRevenueByDayAsync(
        ...
        CancellationToken cancellationToken)
    {
        // Group by UTC date components — translates to SQL on all providers.
        // DateOnly construction happens in C# on the already-aggregated rows (one per day).
        var rows = await db.Sales.AsNoTracking()
            ...
            .Select(g => new
            {
                ...
                g.Key.Day,
                SaleCount = g.Count(),
                GrossRevenue = g.Sum(s => s.GrossTotal),
                Discounts = g.Sum(s => s.ItemDiscountTotal + s.SaleDiscountTotal),
                NetRevenue = g.Sum(s => s.NetTotal)
            })
            .OrderBy(g => g.Year).ThenBy(g => g.Month).ThenBy(g => g.Day)
            .ToArrayAsync(cancellationToken);

        return rows
            .Select(r => new RevenueByDayEntry(
                new DateOnly(r.Year, r.Month, r.Day),
                r.SaleCount,
                r.GrossRevenue,
                r.Discounts,
                r.NetRevenue))
