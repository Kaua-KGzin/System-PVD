...
public sealed class SaleService(PdvDbContext db, FiscalDocumentService fiscalDocumentService)
{
    ...
    public async Task<ServiceResult<SaleResponse>> CreateAsync(CreateSaleRequest request, CancellationToken cancellationToken)
    {
        ...
        var cashSession = await db.CashSessions.FirstOrDefaultAsync(
            session => session.Id == request.CashSessionId,
            cancellationToken);
        ...
        var nextSaleNumber = await db.Sales.Select(sale => (int?)sale.Number).MaxAsync(cancellationToken) ?? 0;
        var sale = new Sale
        {
            Number = nextSaleNumber + 1,
    ...
    public async Task<ServiceResult<SaleResponse>> CancelAsync(Guid id, CancelSaleRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var product = await db.Products.FirstAsync(product => product.Id == item.ProductId, cancellationToken);
